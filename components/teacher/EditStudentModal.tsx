...
export function EditStudentModal({
  ...
  const [state, formAction, isPending] = useActionState(updateStudent, undefined);
  ...
  useEffect(() => {
    if (state?.success && state.student) {
      onStudentUpdated(state.student);
      onClose();
    }
  }, [state, onClose, onStudentUpdated]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
      ...
      >
        ...
        <form action={formAction} className="space-y-8">
          ...
          <div>
            <label htmlFor="edit-name" className="mb-2 block text-sm text-zinc-500">
              Ονοματεπώνυμο
            </label>
            <input
              id="edit-name"
